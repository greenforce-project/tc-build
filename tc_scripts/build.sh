...
./build-llvm.py ${build_flags} \
    --defines LLVM_OPTIMIZED_TABLEGEN=ON LLVM_INCLUDE_TESTS=OFF LLVM_INCLUDE_DOCS=OFF LLVM_INCLUDE_EXAMPLES=OFF LLVM_BUILD_TESTS=OFF LLVM_BUILD_EXAMPLES=OFF LLVM_BUILD_BENCHMARKS=OFF LLVM_ENABLE_BINDINGS=OFF LLVM_ENABLE_ZSTD=ON LLVM_ENABLE_RTTI=OFF LLVM_ENABLE_EH=OFF CLANG_ENABLE_ARCMT=OFF CLANG_ENABLE_STATIC_ANALYZER=OFF LLVM_PARALLEL_COMPILE_JOBS="${cpu_core}" LLVM_PARALLEL_LINK_JOBS="4" LLVM_ENABLE_RUNTIMES="compiler-rt" LLVM_ENABLE_UNWIND_TABLES=OFF LLVM_ENABLE_BACKTRACES=OFF CMAKE_C_FLAGS="-march=native -O3 -pipe" CMAKE_CXX_FLAGS="-march=native -O3 -pipe" \
    --build-stage1-only \
